...
def book():
    if request.method == 'POST':
        ...
        msg = Message(
            ...
            recipients=['yamkelabhaku751@gmail.com']
        )
        msg.body = f'''
A new tutoring session has been booked.
...
Log in to the site for more details.
'''
        try:
            mail.send(msg)
...
def view_material(material_id):
    ...
    if request.method == 'POST' and current_user.is_authenticated:
        ...
        is_positive = request.form.get('is_positive') == 'true'
        if comment_text:
            new_comment = Comment(
                text=comment_text,
                user_id=current_user.id,
